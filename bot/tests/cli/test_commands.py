...
def test_onboard_fresh_install(mock_paths):
    ...
    assert (workspace_dir / "persona" / "memory" / "MEMORY.md").exists()
...
def test_agent_uses_workspace_directory_for_cron_store(monkeypatch, tmp_path: Path) -> None:
    ...
    assert result.exit_code == 0
    assert seen["cron_store"] == config.workspace_path / "persona" / "trigger" / "cron" / "jobs.json"
...
def test_agent_workspace_override_does_not_migrate_legacy_cron(
    ...
    assert result.exit_code == 0
    assert seen["cron_store"] == override / "persona" / "trigger" / "cron" / "jobs.json"
    assert legacy_file.exists()
    assert not (override / "persona" / "trigger" / "cron" / "jobs.json").exists()
...
def test_agent_custom_config_workspace_does_not_migrate_legacy_cron(
    ...
    assert result.exit_code == 0
    assert seen["cron_store"] == custom_workspace / "persona" / "trigger" / "cron" / "jobs.json"
    assert legacy_file.exists()
    assert not (custom_workspace / "persona" / "trigger" / "cron" / "jobs.json").exists()
...
def test_migrate_cron_store_moves_legacy_file(tmp_path: Path) -> None:
    ...
    workspace_cron = config.workspace_path / "persona" / "trigger" / "cron" / "jobs.json"
