...
API_CHAT_ID = "default"
AGENT_LOOP_KEY = web.AppKey("agent_loop", object)
MODEL_NAME_KEY = web.AppKey("model_name", str)
REQUEST_TIMEOUT_KEY = web.AppKey("request_timeout", float)
SESSION_LOCKS_KEY = web.AppKey("session_locks", dict)


def _app_value(app: Any, key: web.AppKey[Any], legacy_key: str, default: Any = None) -> Any:
    if key in app:
        return app[key]
    if hasattr(app, "get"):
...
async def handle_chat_completions(request: web.Request) -> web.Response:
    ...
    timeout_s: float = _app_value(request.app, REQUEST_TIMEOUT_KEY, "request_timeout", 120.0)
    model_name: str = _app_value(request.app, MODEL_NAME_KEY, "model_name", "nanobot")
